...
-- www.leuchtturm-dagebuell.de www.lotsenturm-usedom.de www.wasserturm-badsaarow.de 
\set site 'www.leuchtturm-dagebuell.de'
\set site 'www.lotsenturm-usedom.de'
-- \set site 'www.wasserturm-badsaarow.de'
...
from
    (
        ...
        from
            structures
        WHERE structures.site=:'site'
        order by structures.site
    ) as t;
...
from
    (
        select
            (select site from structures where id=art.str_id) as _site,
            'article' as _type,
            'a' || id as _id,
        ...
        from
            kyff_articles art
        where
            site_id = 9 -- AND _site=:'site'
        order by _site
    )  as t WHERE _site=:'site';